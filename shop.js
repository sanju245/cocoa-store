(() => {
  ...
  if (waNumber) {
    ...
    $('#waFloat').href = `https://wa.me/${waNumber}?text=${encodeURIComponent(`Hi ${storeName}, I have a question.`)}`;
  }
  ...
  async function start() {
    ...
    if (st && st.data) settings = { ...settings, ...st.data };
    applySettings();
    if (!ok) {
  ...
  function cardHTML(p) {
    ...
    return `<article class="card">
      <button type="button" class="card-open" data-id="${esc(p.id)}">
        <span class="card-img">${img ? `<img src="${esc(img)}" alt="${esc(p.name)}" loading="lazy">` : ''}${tag ? `<span class="tag">${esc(tag)}</span>` : ''}</span>
        ...
        <div class="pd-info">
          ...
          <div class="stack">
            ${gone
              ...
              : '<button class="btn btn-gold" type="button" data-act="buy">Buy now</button><button class="btn" type="button" data-act="add">Add to bag</button>'}
          </div>
          <div class="pd-links">
  ...
  function renderCheckout(body) {
    ...
    body.innerHTML = `
      ...
      <form id="checkoutForm">
        ...
        <div id="sumBox"></div>
        <div class="stack">
          ${canPay ? '<button class="btn btn-gold" type="submit" value="razorpay">Pay <span id="payAmt"></span> online</button>' : ''}
  ...
  function renderDone(body) {
    ...
    body.innerHTML = `<div class="done">
      ...
      <p>${esc(text)} Keep the order number to track your order.</p>
      <div class="stack">
  ...
  async function placeOrder(method, form) {
    ...
    if (!order) {
      ...
      const { data, error } = await sb.rpc('place_order', {
        ...
        p_coupon: coupon ? coupon.code : null, p_method: method
      });
      if (error) {
      ...
      order = {
        ...
        discount: Number(data.discount), shipping: Number(data.shipping), customer, phone, address
      };
      pending = order;
    }
    ...
    const finish = (saved, paid, paymentId) => {
      ...
      pending = null;
      store.set('cl_cart', cart);
      doneInfo = { short: order.short, saved, paid, paymentId, waUrl: waNumber ? `https://wa.me/${waNumber}?text=${encodeURIComponent(waMessage({ ...order, paid, paymentId }))}` : '' };
      view = 'done';
      renderCart();
      fetchProducts().then((ok) => { if (ok) { reconcileCart(); renderGrid(); } });
    };
