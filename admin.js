(() => {
  ...
  function renderOrders() {
    ...
    const list = orders.filter((o) =>
      ...
      (!q || `${o.customer_name} ${o.phone} ${o.id.slice(0, 6)}`.toLowerCase().includes(q))
    );
  ...
  $('#exportOrders').addEventListener('click', () => {
    ...
    const head = ['Order', 'Date', 'Name', 'Phone', 'Address', 'Items', 'Subtotal', 'Discount', 'Delivery', 'Total', 'Payment', 'Payment ID', 'Status'];
    const rows = orders.map((o) => [
      o.id.slice(0, 6).toUpperCase(), new Date(o.created_at).toLocaleString('en-IN'), o.customer_name, o.phone,
      (o.address || '').replace(/\s*\n\s*/g, ', '),
